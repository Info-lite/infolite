...
* 帰無仮説$H_0$：各クラスの修学旅行先の希望の傾向は同じである
* 対立仮説$H_1$：各クラスの修学旅行先の希望の傾向には違いがある
...
\begin{align}
  \chi^2 = \sum_i{\sum_j{\frac{(f_{ij}-t_{ij})^2}{t_{ij}}}}
\end{align}

#### 自由度の算出

自由度$\varphi$は以下の数式で求まります。
...
{% screenshot 05_04newline.png "[クラス]の列を作成" %}

{% screenshot 05_05newline.png "[修学旅行先]の列を作成" %}

{% screenshot 05_06newline.png "[人数]の列を作成" %}

{% screenshot 05_07input.png "データを入力" %}

&#9313; メニュー[分析] - [二変量の関係]を選択します。

{% screenshot 05_08analysis.png "[二変量の関係]を選択" %}

&#9314; [列の選択]の中の[クラス]をクリック - [X,説明変数]をクリック - [列の選択]の中の[修学旅行先]をクリック - [Y,目的変数]をクリック - [列の選択]の中の[人数]をクリック - [度数]をクリック - [OK]をクリックします。

{% screenshot 05_09analysis.png "[クラス]を[X,説明変数]、[修学旅行先]を[Y,目的変数]、[人数]を[度数]に割り当て" %}

{% screenshot 05_10analysis.png "二変量の関係 分析結果" %}

&#9315; [▼分割表]の[▼]をクリック - [度数]以外のチェックを外します。

{% screenshot 05_11analysis.png "度数のみチェック" %}

#### 出力結果

{% screenshot 05_12analysis.png "Pearsonのカイ二乗検定 $p$値<.0001" %}

$p$値 $<$ 0.0001 が求まりました。$p$値 $<$ 0.0001 $<$ 有意水準$\alpha$ = 0.05 なので、帰無仮説$H_0$は棄却されます。
...
>   表 1 に関して、$\chi^2$検定を行ったところ、有意差が認められた($\chi^2(6) = 51.358,\ p < .05$)。
...
{% screenshot 05_13analysis.png "[偏差]をチェック" %}

#### 出力結果

{% screenshot 05_14analysis.png "分割表 残差表示" %}
...
{% screenshot 05_15chart.png "[チャート]をクリック" %}

&#9313; [列の選択]の中の[人数]をクリック - [統計量]をクリック - [データ]をクリック - [列の選択]の中の[修学旅行先]をクリック - [カテゴリ,X,水準]をクリックします。

{% screenshot 05_16chart.png "[人数] - [統計量] - [データ]をクリック" %}

{% screenshot 05_17chart.png "[人数]を[統計量]、[修学旅行先]を[カテゴリ,X,水準]に割り当て" %}

&#9314; [ $>$ その他の役割]をクリック - [列の選択]の中の[クラス]をクリック - [グループ変数]をクリック - [OK]をクリックします。

{% screenshot 05_18chart.png "[その他の役割]をクリック" %}

{% screenshot 05_19chart.png "[クラス]を[グループ変数]に割り当て" %}

#### 出力結果

{% screenshot 05_20chart.png "棒グラフ クラスごとの修学旅行先のパターン" %}
...
{% screenshot 05_21analysis.png "[対応分析]をチェック" %}

#### 出力結果

{% screenshot 05_22analysis.png "対応分析表示" %}
