...
t分布は、正規分布同様の釣鐘型をしていますが、正規分布とは異なり、自由度$\varphi$（ファイ）によって形状を変えます。
...
なお$\bar{x}$は標本平均、$\mu_0$は母平均、$s$は標本の標準偏差、$n$は標本の大きさ（サンプルサイズ）です。


### $p$値の算出

有意水準$\alpha$と比較する確率$p$値を計算します。
$p$値は、自由度$\varphi$のt分布において、$\left | t \right | $以上の値が発生する確率です。
...
なお、$\bar{x}$は標本平均、$s$は標本の標準偏差、$n$は標本の大きさ（サンプルサイズ）、$t_{n-1}( \alpha )$は、自由度$n-1$のt分布における100$\times \alpha $%点を示します。
