...
対になったデータx_iとy_i(i = 1, 2, &hellip; ,n)から単回帰式y = a + bx（a：切片、b：回帰係数）を求めるには以下の数式を用います。
...
目的変数yの変動のうち、説明変数xによって説明できる割合を寄与率R_2、もしくは決定係数R_2といいます。
...
単回帰分析では、寄与率R_2 = (相関係数r)<sup>2</sup> となります。

#### $p$値の算出

有意水準$\alpha$ と比較する確率$p$値を計算します。$p$値はF分布において、F以上の値が発生する確率です。
...
$p$値$<$ .0001*が求まっています。
$p$値$<$ .0001$<$ 有意水準$\alpha$ = 0.05なので、帰無仮説$H_0$は棄却されます。
