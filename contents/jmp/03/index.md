...
回帰直線モデル(y = a<sub>0</sub>＋a<sub>1</sub>x<sub>1</sub>＋…＋a<sub>n</sub>x<sub>n</sub>)ではyに相当します。</dd>
<dt>説明変数(x)：独立変数</dt>
<dd>目的変数に影響を与えていると考えられる変数のことです。物事の原因とも言えます。独立変数とも呼ばれます。
回帰直線モデル(y = a<sub>0</sub>＋a<sub>1</sub>x<sub>1</sub>＋…＋a<sub>n</sub>x<sub>n</sub>)ではx<sub>1</sub>…x<sub>n</sub>に相当します。</dd>
...
* 帰無仮説H0：&mu; A = &mu; BＡ組とＢ組では平均点に差があるとはいえない
* 対立仮説H1：&mu; A≠&mu; BＡ組とＢ組では平均点に差がある
...
p値&le;有意水準&alpha; &rarr; 帰無仮説H0を棄却する。
p値 &gt; 有意水準&alpha; &rarr; 帰無仮説H0を棄却しない。
...
* 帰無仮説H0：&mu; A = &mu; BＣ組とＤ組では平均点に差があるとはいえない
* 対立仮説H1：&mu; A≠&mu; BＣ組とＤ組では平均点に差がある
...
Welchの分散分析の部分から、p値 = 0.0248 が求まりました。
...
* 帰無仮説H0：&mu; B - &mu; A = 0後期の成績は前期の成績より向上しているとはいえない
* 対立仮説H1：&mu; B - &mu; A &gt; 0後期の成績は前期の成績より向上している
...
母平均の差 &mu;B-&mu;A の (1-&alpha;)×100％ 信頼区間は、以下の通りです。
...
母平均の差&mu; B - &mu; Aの 95 ％信頼区間は、2.03269  &lt; &mu; B - &mu; A  &lt; 10.3006 になります。
