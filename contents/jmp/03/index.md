...
回帰直線モデル $(y = a_0 + a_1 x_1 + ... + a_n x_n)$ では $y$ に相当します。</dd>
<dt>説明変数($x$)：独立変数</dt>
<dd>目的変数に影響を与えていると考えられる変数のことです。物事の原因とも言えます。独立変数とも呼ばれます。
回帰直線モデル $(y = a_0 + a_1 x_1 + ... + a_n x_n)$ では $x_1 ... x_n$ に相当します。</dd>
</dl>
...
有意水準と比較する確率$p$値を求めます。$p$値は、自由度$ \varphi A + \varphi B $のt分布において、$|t|$以上の値が発生する確率です。
...
$p$値 = 0.0073 が求まりました。よって、$p$値 = 0.0073 $<$ 有意水準$\alpha$ = 0.05 であるので、帰無仮説$H_0$は棄却されます。
