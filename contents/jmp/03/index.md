...
* 帰無仮説H0：μA＝μB　Ａ組とＢ組では平均点に差があるとはいえない
* 対立仮説H1：μA≠μB　Ａ組とＢ組では平均点に差がある
...
SA、SB＝各グループの平方和、nA、nB＝各グループのデータ数、φA＝nA－1、φB＝nB－1
...
有意水準と比較する確率p値を求めます。p値は、自由度φA+φBのt分布において、|t|以上の値が発生する確率です。
...
p値＝0.9301 が求まりました。設定した有意水準αは 0.05 です。よって、p値＝0.9301＞有意水準α＝0.05 であるので、等分散性があることがわかりました。

7. 続いて、[▼クラスによる点数の一元配置分析]の[▼]をクリック - [平均/ANOVA/プーリングしたt検定]を選択します。
![](./pic/03_12analysis.png)
出力結果
![](./pic/03_13analysis.png)
p値＝0.0413 が求まりました。設定した有意水準αは 0.05 です。よって、p値＝0.0413＜有意水準α＝0.05 であるので、帰無仮説H0は棄却されます。したがって、Ａ組とＢ組で点数の母平均には差があると判断します。
...
--------------
母平均の差μA－μBの(1-α)×100 ％信頼区間は、以下の式で求められます。
...
"t検定"から"差の上側信頼限界"と"差の下側信頼限定"を見ます。母平均の差μA－μBの 95 ％信頼区間は、0.813＜μA－μB＜36.217 となります。
...
* 帰無仮説H0：μA＝μB　Ｃ組とＤ組では平均点に差があるとはいえない
* 対立仮説H1：μA≠μB　Ｃ組とＤ組では平均点に差がある
...
有意水準と比較する確率p値を求めます。p値は、自由度φ*において、|t|以上の値が発生する確率です。
...
1. まず、表 2 を入力しましょう。練習 1 と同じように、 1 列目は[列名]に"クラス"と入力し、データタイプを[文字]、尺度を[名義尺度]
2 列目は[列名]に"点数"と入力し、データタイプを[数値]、尺度を[連続尺度]
このように列を作成してください。
...
Welchの分散分析の部分から、p値＝0.0248 が求まりました。よって、p値＝0.0248＜有意水準α＝0.05 であるので、Ｃ組とＤ組では点数の母平均に違いがあると判断されたことになります。
...
* 帰無仮説H0：μB－μA＝0　後期の成績は前期の成績より向上しているとはいえない
* 対立仮説H1：μB－μA＞0　後期の成績は前期の成績より向上している
...
有意水準と比較する確率p値を求めます。p値は、自由度φ＝n－1 のt分布において、|t|以上の値が発生する確率です。
...
1. まず、表 3 を入力します。 [列名]に"前期"と入力し、データタイプは[数値]、尺度は[連続尺度]
[列名]に"後期"と入力し、データタイプは[数値]、尺度は[連続尺度]
このように列を作成してください。
...
p値＝0.0073 が求まりました。よって、p値＝0.0073＜有意水準α＝0.05 であるので、帰無仮説H0は棄却されます。前期の平均点 60.5833 と後期の平均点 68.75 には有意差があることがわかり、後期試験の成績（Ｂ）は、前期試験の成績（Ａ）よりも向上していると判断できます。


2つの母平均の差の推定（対応のあるデータ）
--------------
母平均の差μB－μAの(1－α)×100 ％信頼区間は、以下の通りです。
...
"上側95％信頼限界"と"下側95％信頼限界"を読みます。母平均の差μB－μAの 95 ％信頼区間は、2.03269＜μB－μA＜10.3006 になります。この間に 95 ％の確率で母平均の差があることになります。
