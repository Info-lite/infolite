...
$p$値 = 3.483 × 10<sup>-8</sup>が求まりました。下図の塗りつぶされた領域が全体に対してpの割合になっています。

{% screenshot 07_practice2_f.png " " %}

$p$値 = 3.483 × 10<sup>-8 </sup>&lt; 有意水準$\alpha$ = 0.05 なので帰無仮説H<sub>0</sub>は棄却されます。したがって、回帰式は統計的に有意であると分かります。
...
予測する側の複数の変数（説明変数$ x$）から予測される側の変数（目的変数$ y$）に当てはまる式を立てると、次のようになります。  
...
$p$値 = 1.247 × 10<sup>-8 </sup>&lt; 有意水準$\alpha$ = 0.05 なので帰無仮説H<sub>0</sub>は棄却されます。したがって、回帰式は統計的に有意であると分かります。
