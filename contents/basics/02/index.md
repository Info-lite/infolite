...
**情報処理教室の"マイドキュメント"内のファイルでは、この部分は該当しません。一般的な利用法として理解しておいてください。**
...
情報処理教室の環境ではこれに加えて、マイドキュメントでもゴミ箱の機能が適用されません。つまり、マイドキュメントのファイルを消去するためごみ箱にドラッグすると、
...
情報処理教室では、マイドキュメントはコンピュータからも見ることができるようになっています。[コンピュータ]-[H:¥]と開いた中身はマイドキュメントの中身と同じになっているはずです。
...
<p>情報処理教室の端末では、デスクトップやお気に入りに保存したデータはログアウト時に削除される仕組みとなっています。自分で作成したデータは必ずマイドキュメントに保存する習慣を身に付けましょう。</p>
</div>
