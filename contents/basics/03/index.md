...
<dd>WWWにおける情報はInternet Explorerなどの"Webブラウザ"を使うことで誰でも自由に閲覧することができます。この情報は"HTML（HyperText Markup Language）"という簡単な言語をつかって記述されているため、誰でも手軽に情報を発信することもできます。また、文章中に設定されている"ハイパーリンク"によって、他の情報へ簡単に移動することができることも特徴のひとつです。この機能によって情報同士が複雑に結び付けられ、まるでクモの巣のようになっていることから"Web"という名前がつけられているのです。みなさんが"インターネット"という言葉で連想するのはこのサービスのことでしょう。</dd>
...
情報基礎教室のパソコンでは、Webを閲覧するソフト（Webブラウザ）として"Microsoft Internet Explorer（IE）"が用意されています。
...
インターネットで検索できるのは電子情報だけではありません。例えば、首都大学東京図書館のウェブサイトからは、本学に収蔵されている書籍を中心に、レポート作成や研究に有用な資料の場所を検索することができます。蔵書を検索するためには、図書館のトップページから[蔵書検索]で検索画面に移ることができます。

-   [首都大学東京図書館](http://www.lib.tmu.ac.jp/)
-   [図書情報検索実習資料](library.pptx)

図書館には、朝日新聞の全文検索など、インターネットで検索できず図書館のPCからのみ利用できるサービスもあります。これらの詳細な使い方は、入学時に配布された小冊子「図書・学術情報をオンラインで利用しよう」を活用して下さい。なくした人は図書館で入手できるほか、以下のリンクからPDF版を閲覧できます。
...
次は実際に首都大学東京図書館で検索をしてみましょう。今回はブルーバックス（書籍）の「材料化学の最前線」を探してみます。まず、首都大学東京図書館のページを開いてください。

-   [首都大学東京図書館](http://www.lib.tmu.ac.jp/)

&#9312; まず、画面中央付近の水色の蔵書検索ボックスの"詳細検索"をクリックします。

![](./pic/opac1.png)

&#9313; WebOPACの詳細検索画面になるので、調べたい本のタイトルを入力します。今回は「材料化学の最前線」と入力しましょう。

![](./pic/opac2.png)

&#9314; 結果が表示されました。このうち探していた「材料化学の最前線 : 分子一個、原子一個をどう操るか」の書籍名をクリックします。

![](./pic/opac3.png)

&#9315; すると、「材料化学の最前線 : 分子一個、原子一個をどう操るか」は本館（南大沢）の 3 階一般書架もしくは日野の小型本コーナーに貸し出し用のものがあるようです。（本館 1 階フロアのものは貸し出ししていません。）

![](./pic/opac4.png)

さらに、"CiNii Books"を利用して「材料化学の最前線」を所蔵している図書館を探してみます。

1. CiNii Books で検索すると所蔵図書館が 243 見つかります。
1. 青山学院大学万代記念図書館(相模原分館) が適当だと判断して、青山学院大学のOPAC を使って書名で検索します。
1. 相模原分館 の一般開架図書に 2 冊あって、 2 冊共に貸し出し可能であることまでわかります。(2013.4.5 現在)
    -   他大学に行って図書館に入るには、首都大図書館からの紹介状が必要ですので図書館の受付で相談して下さい。
    -   首都大図書館を通じて他機関への複写依頼（有料）もできます。
