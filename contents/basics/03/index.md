...
Webブラウザでウェブサイトを見るときには、自分が見たいページのアドレスをアドレスバーに入力します。このアドレスは、"URL（Uniform Resource Locator）"の形式であらわされています。例えば首都大学東京のウェブサイトを見る場合、入力するアドレスは"http://www.tmu.ac.jp/"となります。今まで説明してきたことを踏まえて考えると、これは「日本（jp）の学術団体（ac）である首都大学東京（tmu）にあるWebサーバ（www）にいって、WWWのプロトコル（http）で情報を見る」という意味になっているわけです。
