...
$p$ 値は、自由度 $\varphi_A + \varphi_B$ の$t$分布において、$|t|$ 以上の値が発生する確率です。
...
&#9316; したがって、検定を行うにはstudentの$t$検定を用います。データ分析から今度は[$t$検定 : 等分散を仮定した2標本による検定]を選択します。
...
したがって、今回の$t$検定では[$t$検定 : 分散が等しくないと仮定した2標本による検定]を用います。
