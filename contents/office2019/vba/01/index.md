...
&#9314; ![上書き保存](pic/filebutton.png)をクリックします。

&#9315; ![Sub/ユーザーフォームの実行](pic/filebutton.png)をクリックします。

{% screenshot xxx.png "マクロの実行方法" %}

成功すると、以下のようにダイアログが表示されます。

{% screenshot xxx.png "HelloVBAの実行結果" %}
...
3.で実行した以下のマクロ「HelloVBA」は、このように説明できます。
...
{% screenshot xxx.png "Excelの表の構造" %}

VBAでは、これらのブックとシートをオブジェクトとして指定して、表を扱うことができます。以下のように使用したいブックをWorkbooks(“ブック名”)、使用したいシートをWorksheets(“シート名”)、使用したいセルをCells(行番号, 列番号)もしくはRange(“セルの名前”)で指定することができます。

{% screenshot xxx.png "Excelの表の構造をVBAで表した図" %}
...
上記は両方とも「u1234567氏名.xlsm」というブックファイルの「Sheet1」というシートのセル名「A1」を示しています。Cellsを用いる際は、行番号がアルファベットではなく、数字に置き換わることに注意が必要です。また、同一のブック内のみで処理を行う場合は、Workbooksは省略することができます。一定の条件に従ってWorksheetsも省略することができますが、厳密に正しく操作しないとプログラムが動かなくなるので、Worksheetsは省略しない方が良いでしょう。
...
{% screenshot xxx.png "PerformanceTableの実行結果" %}
...
プログラミングでは、同じ結果を求めるために複数の方法があります。PerformanceTableのRange()をCells()で同じ内容になるように書き換えてみましょう。この際に、一度、各セルの値をクリアしてから行うとわかりやすいでしょう。
