...
無限ループが起こる原因については、以下の参考資料を確認してみてください。

{% appendix ../appendix/EndlessLoop.html "無限ループ" %}
...
なお、Cellsを用いて、列番号をわかりやすくするためには、ワークシートの見た目をR1C1参照形式に切り替えることもできます。以下の参考資料にやり方が載っていますので、必要に応じて切り替えてみてください。

{% appendix ../appendix/R1C1.html "ワークシートのR1C1参照形式" %}
...
法則性を考えて、Aさん～Eさんの処理を行うことを考えると、行番号が変わっていく繰り返しをすることになるため、初期値はAさんの行番号である3、終了値はEさんの行番号である7となります。1行ごとにそれぞれの人の成績が入力されているため、加算値を1にすることによって、1行ごとに計算をしていく繰り返しができます。
...
1.5.練習問題2では、行番号にカウンターとなる変数を用いた繰り返しの練習問題を行いました。表の各セルを順番に読んで、処理を行っていくという手法は、Excelを用いたVBAの繰り返しでよく行われることであり、この方法は、行番号だけではなく、列番号にカウンターとなる変数を用いて行われることもあります。

また、繰り返しは、二重・三重と重ねることができます。行番号と列番号の両方を繰り返しすることによって、Excelの表をより効率的に処理することができるようになります。繰り返しを重ねることは、繰り返しや表の構造、変数を正確に理解し、アルゴリズムをなければ難しいことですが、最終的には二重の繰り返しを用いて、行番号と列番号の両方を繰り返すことができるようになりましょう。

十分に繰り返しについて理解できたら、以下の参考資料を用いて勉強してみると良いでしょう。

{% appendix ../appendix/DoubleFor.html "二重の繰り返し" %}
...
以下の成績表について、PerformanceTable8を改良して、合計点と平均点に加えて、国語と英語と数学の評価を判定するプログラムを作成してみましょう。各人の評価については、繰り返しを1回用いて、自動で同じように評価を算出できるようにしましょう。

{% screenshot Test04_1_Pre.png "Aさん～Eさんの成績表" %}
...
以下の手順と条件を満たすマクロを作成しましょう。

* セルA1に好きな整数を入力する。
* マクロを実行すると、セルB1にセルA1の整数の階乗を入力する。
...
* セルA1に好きな整数を入力する。
* マクロを実行すると、以下の処理を行う。
    * セルB1にセルA1から1引いた整数を入力する。
    * セルB2にセルB1から1引いた整数を入力する。
    * 上記の法則にしたがって行方向にセルB3、セルB4…と、入力する整数が1になるまで整数を入力する。
...
* セルA1に好きな整数を入力する。
* 作成したマクロを実行すると、セルA1に入力した整数が素数かどうかを判定する。
    * セルA1に入力した整数が素数である場合は、「{セルA1}は素数です。」とダイアログ表示する。
    * セルA1に入力した整数が素数ではない場合は、「{セルA1}は素数ではありません。」とダイアログ表示する。

※{セルA1}は、それぞれのセルに入力してある値に置き換える。

### 課題5

以下の成績表について、PerformanceTable8を改良して、合計点と平均点に加えて、国語と英語と数学の評価を判定するプログラムを作成してみましょう。各人の評価については、行番号と列番号で二重の繰り返しを用いて、自動で同じように評価を算出できるようにしましょう。
