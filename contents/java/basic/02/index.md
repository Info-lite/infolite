...
`データ型 変数名;`の形で宣言します。
上のプログラムでは、3行目が変数宣言にあたります。

さらに、以下のように宣言と同時に初期値を入力することもできます。

    int x=3;

変数宣言は、プログラムの任意の場所で行えますが、特にプログラム全体を通して使う変数は**先頭にまとめる**ほうが良いでしょう。
...
4行目の `x=3;` の `=` は、**値の代入**をせよという意味です。数学で使う=とは意味が少し違います。

プログラミングの場合、`x=3;`は**xを3に変更せよ**という命令になります。
...
4行目を見てみましょう。`x=1+2;`とあるように、代入するときの右辺には **数式** も使えます。
<br>1+2を先に計算し、3という数字がxの値になります。
