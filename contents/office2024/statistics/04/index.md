...
&#9315; 男性のグラフを選択した状態で、[グラフのデザイン]リボンの[データの選択]をクリックします。
...
グラフを選択した状態で[グラフのデザイン]リボンから[グラフ要素を追加]をクリック、[凡例]にカーソルを合わせて表示したい位置を選択します。
...
今回は標本の大きさ（サンプルサイズ）が少ないので自分でも数えられますが、標本が大きい場合のことも考えて、`COUNTA` 関数を使います。

[数式]リボン - [関数の挿入] から `COUNTA` 関数を選択し、男性の身長の全データを選択範囲として、男性の人数を数えます。

{% screenshot 06_36count.png "counta" %}

出力結果

{% screenshot 06_37count.png "countaの出力結果" %}
