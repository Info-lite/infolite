...
![基本統計量](pic/03_02table.png)

まずは式を構成する値を計算します。

&#9312; 列Dを選択し、[ホーム]リボン - [セル]タブの [挿入] から [シートの列を挿入] を2回クリックします。

![シートの列を挿入1](pic/03_03var01.png)

![シートの列を挿入2](pic/03_04var02.png)

&#9313; "D1" に "平均からの差"、"E1" に "差の2乗" と入力します。入力したテキストが見えるように列の幅を調節します。

![列の幅の調整](pic/03_05var03.png)

&#9314; "D2" に `=A2-$H$2` と入力します。絶対参照 [F4] は[過去の資料](../../excel/02/index.html)を参考にしてください。

![平均からの差1](pic/03_06var04.png)

![平均からの差2](pic/03_07var05.png)

&#9315; また、"E2" に `=D2^2` と入力します。

![差の二乗1](pic/03_08var06.png)

![差の二乗2](pic/03_09var07.png)

&#9316; "D2" と "E2" セルを選択し、オートフィルを使って、全ての国語の点数における平均からの差と差の2乗を計算します。

![平均からの差と差の二乗のオートフィル1](pic/03_10var08.png)

![平均からの差と差の二乗のオートフィル2](pic/03_11var09.png)

&#9317; "D22" セルに "差の2乗の総和" と入力します。

![差の2乗の総和1](pic/03_12var10.png)

&#9318; "E22" セルに `=sum(E2:E21)` と入力します。

![差の2乗の総和のオートフィル1](pic/03_13var11.png)

![差の2乗の総和のオートフィル2](pic/03_14var12.png)

これで分散を計算する準備が整いました。

&#9319; "G17" セルと"G18"セルに、それぞれ"不偏分散"、"標本分散"と入力します。

![不偏分散と標本分散](pic/03_15var13.png)

&#9320; "H17" セルに `=E22/(H14-1)` と入力します。

![不偏分散](pic/03_16var14.png)

![不偏分散算出後](pic/03_17var15.png)

不偏分散が求まりました。

&#9321; 次に、"H18" セルに `=E22/H14` と入力します。

![標本分散](pic/03_18var16.png)

![標本分散算出後](pic/03_19var17.png)

標本分散が求まりました。

ここで、"M8" セルの値と "H17" セル、"H18" セルの値を見比べてみます。

![データ分析の分散](pic/03_20var18.png)
...
![標準偏差（標本）の入力](pic/03_21sd01.png)

&#9313; "H20" セルに `=SQRT(H17)` と入力します。

![標準偏差（不偏）](pic/03_22sd02.png)

![標準偏差（不偏）算出後](pic/03_23sd03.png)

&#9314; オートフィルを用いて "H21" に標本の標準偏差を求めます。

![標準偏差のオートフィル](pic/03_24sd04.png)
...
![有意水準](pic/03_25mean.png)

### t分布表

検定統計量t値は下記の数式から求めることができます。

![統計検定量t](pic/03_26t.png)

<span><img src="pic/03_27x.png" /></span>は標本平均、<i>&mu;<sub>0</sub></i> は母平均、<i>s</i> は標本の標準偏差、<i>n</i> は標本数です。

![t分布自由度 3](pic/03_28t3.png)

![t分布自由度 100](pic/03_29t100.png)
...
![有意水準α](pic/03_31analysis01.png)

&#9313; "H24" セルに `=TINV(H23,H14-1)` と入力し、t値を算出します。

![t値の算出](pic/03_32analysis02.png)

![t値算出後](pic/03_33analysis03.png)
...
![信頼区間](pic/03_34ave.png)

<span><img src="pic/03_27x.png" /></span>は標本平均、<i>s</i> は標本の標準偏差、<i>n</i> は標本数、
